...
def des_encrypt(plaintext_in, IP, key):
    permutated_plaintext_in = permutation(text_to_binary(plaintext_in), IP)
    ...
    xored_right = xor(right_expanded, key)  # Pass key to xor
    sbox_result = sbox(xored_right)

    permutated_left = permutation(first_half , P)
    xored_left = xor(first_half , permutated_left)

    final_result = sbox_result + xored_left

    print(final_result)

des_encrypt(plaintext, IP, key)